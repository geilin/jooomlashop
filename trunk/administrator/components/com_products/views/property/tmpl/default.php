...
defined( '_JEXEC' ) or die( 'Restricted access' ); 
		...
		JToolBarHelper::title( JText::_('property'), 'generic.php');
		JToolBarHelper::publishList('publish');
		JToolBarHelper::unpublishList('unpublish');
		JToolBarHelper::editList('edit');
		JToolBarHelper::deleteList('Are you sure you want to remove property ?','remove');
		JToolBarHelper::addNew('add');
?>
<div>Total Property: <?php echo $this->total; ?></div>
<br/>
...
<tr>
<th width="20">
	<input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count( $this->properties ); ?>);" />
</th>
<th class='title'><?php echo JText::_('Name'); ?></th>
<th width="7%"><?php echo JText::_('Ordering'); ?><a href="javascript:saveorder(1, 'saveOrder')" title="Save Order property"><img src="images/filesave.png" alt="Save Order"  /></a></th>
...
$k = 0;
global $option;
for ($i=0, $n=count( $this->properties ); $i < $n; $i++)
{
	$row = $this->properties[$i];
	$checked = JHTML::_('grid.id', $i, $row->id );
	$link = JFilterOutput::ampReplace( 'index.php?option=com_products&controller=property&task=edit&cid[]='. $row->id );
